...
# -*- coding: utf8 -*-
"""
说明: 环境变量`FK_UP`账号密码`-`分割
cron: 20 */6 * * *
new Env('Faker Engine-签到');
"""
import requests
import os
...
def login(usr, pwd):
    ...
    login_url = 'https://www.fakerengine.com/wp-json/jwt-auth/v1/token'
    headers = {
    ...
    if res.status_code == 200:
        ...
        check_url = 'https://www.fakerengine.com/wp-json/b2/v1/userMission'
        check_head = {
            'authorization': f'Bearer {token}',
            'origin': 'https://www.fakerengine.com',
            'referer': 'https://www.fakerengine.com/task',
            'user-agent': 'Mozilla/5.0 (Linux; Android 10; PBEM00) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.52 Mobile Safari/537.36'
...
if __name__ == '__main__':
    i = 0
    if 'FK_UP' in os.environ:
        users = os.environ['FK_UP'].split('&')
        for x in users:
            i += 1
            name, pwd = x.split('-')
        ...
        tt = '\n'.join(List)
        print(tt)
        send('Faker Engine', tt)
    else:
        print('未配置环境变量')
        send('Faker Engine', '未配置环境变量')